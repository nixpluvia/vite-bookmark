...
function createInitialForm({ editTarget, initialUrl, activeCategory, initialImageUrl }) {
	if (editTarget) {
 ...
 * @param {string}    initialUrl  - Ctrl+V 로 파싱된 URL (새 추가 시)
 * @param {object}    editTarget  - 편집 대상 북마크 (편집 모드)
 * @param {string[]}  categories  - 카테고리 목록 ('전체' 제외)
...
export default function Modal({
	...
	const [form, setForm] = useState(() =>
		createInitialForm({ editTarget, initialUrl, activeCategory, initialImageUrl }),
	);
    ...
    if (form.thumbnail === "" && initialImageUrl && initialImageUrl !== form.thumbnail) {
	...
	return (
		...
		<div
			role="dialog"
			aria-modal="true"
			aria-label={isEdit ? "북마크 편집" : "북마크 추가"}
